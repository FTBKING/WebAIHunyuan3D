...
public class MeshProcessingController {
    ...
    public Result generateMvWhiteMesh(@ModelAttribute WhiteMeshRequestDTO whiteMeshRequestDTO) {
        log.info("生成白模接口被调用");
        //调用逻辑层方法，返回url
        String url = meshProcessingService.generateMvWhiteMesh(whiteMeshRequestDTO);
        //判断是否生成成功
        if (url == null) {
